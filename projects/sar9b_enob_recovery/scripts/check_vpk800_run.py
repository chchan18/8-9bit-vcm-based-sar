#!/usr/bin/env python3
"""Check, archive, and summarize the SAR9B Vpk=800m Maestro run."""
...
RUN_DIR = Path("projects/sar9b_enob_recovery/runs/vpk800_p2200_baseline")
MANIFEST = RUN_DIR / "run_start_manifest.json"
...
def remote_dir_exists(client: VirtuosoClient, path: str) -> bool:
    return ssh(client, f"test -d {path} && echo yes || echo no", timeout=20).strip() == "yes"


def remote_tail(client: VirtuosoClient, path: str, lines: int = 40) -> str:
...
def check_once(client: VirtuosoClient, manifest: dict, archive: bool) -> dict:
    ...
    run_log_tail = remote_tail(client, run_log, lines=60)
    spectre_tail = remote_tail(client, spectre_out, lines=100)
    ...
    status = {
        ...
        "psf_exists": remote_dir_exists(client, results_dir),
        "netlist_exists": remote_file_exists(client, netlist),
        "spectre_out_exists": remote_file_exists(client, spectre_out),
        "completed": completed,
    ...
    if archive:
        ...
        download_if_exists(client, netlist, RUN_DIR / "input.scs")
    ...
    if successful:
        complete = {
            ...
            "target_vpk": manifest["target_vpk"],
            "history": history,
            "run_log_remote": run_log,
            "results_dir_remote": results_dir,
            "netlist_remote": netlist,
        ...
        (RUN_DIR / "run_complete_manifest.json").write_text(
        ...
        json.dumps(
            {
                ...
                "spectre_out_exists": status["spectre_out_exists"],
                "run_errors": status["run_errors"],
                "spectre_summary": status["spectre_summary"],
                "active_processes": status["active_processes"],
                "run_log_tail": status["run_log_tail"].splitlines()[-10:],
                "spectre_tail": status["spectre_tail"].splitlines()[-10:],
            },
            indent=2,
        ),
        flush=True,
